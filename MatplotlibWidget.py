...
class MyMplCanvas(FigureCanvas):
    ...
    def update_figure(self, datay, datax):
        self.axes.set_ylabel('loss')
        self.axes.set_xlabel('iter')
        if self.curveObj is None:
            # create draw object once
            self.curveObj, = self.axes.plot_date(np.array(datax), np.array(datay), 'bo-')
        else:
            ...
            self.axes.set_xlim(datax[0], datax[-1])
            self.axes.set_ylim(0, max(datay))
        self.draw()
        self.axes.grid(True)
        self.draw()
